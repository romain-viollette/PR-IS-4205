...
void setup() {
  ...
  for(int i=0; i<NUMREADINGS; i++)   readings1[i] = 0;  // initialize readings to 0
  ...
  for(int i=0; i<NUMREADINGS; i++)   readings2[i] = 0;  // initialize readings to 0
...
void getMotorData()  {                                                        // calculate speed, volts and Amps
  ...
  // 1 tour de roue = 360 impulsions
  // roue de 11,8mm de diamètre -> circonférence de 11,8*pi=37,07mm
  // vitesse = NbrImpulsion
  // 1 tour de roue en 1 secondes -> 37,07mm/s
  speed_act1 = ((count1 - countAnt1)*(60*(1000/LOOPTIME)))/(16*30);          // 16 pulses X 29 gear ratio = 464 counts per output shaft rev
  countAnt1 = count1;                  

  speed_act2 = ((count2 - countAnt2)*(60*(1000/LOOPTIME)))/(16*30);          // 16 pulses X 29 gear ratio = 464 counts per output shaft rev
  countAnt2 = count2;                  
