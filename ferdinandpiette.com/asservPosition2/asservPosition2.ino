...
void computeSpeedPolicy()
{
  ...
  static const byte       ARRET_ =             0;
  static const byte       ACCELERATION_ =      1;
  static const byte       CROISIERE_ =         2;
  static const byte       DECELERATION_ =      3;

  static byte etat = ARRET_;
  ...
  else if(roueG.regime == ARRET || roueG.consigne_moteur == roueG.consigne_finale)
  {
    etat = ARRET_;
  }
  else
    if(roueG.consigne_moteur < (roueG.consigne_finale/2)) // avant la moitié du parcours
    {
    ...
    if(roueG.consigne_moteur < (roueG.consigne_finale - alpha)) //
    {
      etat = CROISIERE_;
    }
    else
  ...
  switch(etat)
  {
    ...
    case ACCELERATION_:
      ...
      roueG.consigne_moteur +=increment;
      alpha +=increment;
    ...
    case DECELERATION_:
      ...
      if(roueG.consigne_moteur > roueG.consigne_finale )
        roueG.consigne_moteur= roueG.consigne_finale ;
    ...
    case ARRET_:
      ...
      if(_DEBUG_CONSIGNE) 
        ...
        if(abs(roueG.tick_codeuse - roueG.consigne_finale)<2)
        {
          roueG.regime = ARRET;
          roueD.regime = ARRET;
          alpha = 0;
  ...
  if(_DEBUG_CONSIGNE)
  {
    ...
    Serial.print("   ");
    Serial.print(roueG.consigne_moteur,DEC);
    Serial.print("   ");
    Serial.print(roueG.tick_codeuse,DEC);
    Serial.print("   ");
    Serial.print(alpha,DEC);
    Serial.print("   ");
    Serial.println(roueG.consigne_finale,DEC);
  }


  roueD.consigne_moteur = roueG.consigne_moteur;
...
{ 
  ...
  if(roueG.regime == ARRET)
  {
    ...
    analogWrite(PWM1, 0);
    roueG.sortie = 0;
    roueG.consigne_moteur = 0;
    roueG.consigne_finale = 0;
  ...
  if(roueD.regime == ARRET)
  {
    ...
    analogWrite(PWM2, 0);
    roueD.sortie = 0;
    roueD.consigne_moteur = 0;
    roueD.consigne_finale = 0;
...
void asservissement(Roue *roue)
{
  ...
  //float erreur = (float)roue->consigne_moteur - roue->nb_tour_par_sec;
